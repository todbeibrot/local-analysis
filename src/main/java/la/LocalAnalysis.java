...
public class LocalAnalysis{
	...
	//static variables:
	final private static int BOARDSIZE = 19;		//it's currently not planned to support other board sizes
	...
	private int[] wallCornerPoint;	//Corner Points for wall. 0=left bottom,1=right bottom,2=left top,3=right top
	private int[] alivePoints;		//stones which you can mark. at the start of the analysis they will connect to a living group on the outside.
									//they represent the possibility to run out with a group. so the inner player is save if he reaches this spot.
	...
	private int wallDistance;		//between analyzed position and surrounding stones there will be a void so the wall will not affect the local position
	private int blackKoThreats;		//builds Ko threats for Black if positive and for White if negative
	private int maxKoThreatsBlack;			//maximal amount of Ko threats for Black. depends on size of Whites eye area.
	private int maxKoThreatsWhite;			//analog for White
	private int mirror;				//if some bugs appear we can mirror the position then start again and mirror back, range from 0 to 7
	private boolean blackToMove;			//true = Blacks turn, false = Whites turn
	private boolean playerInsideBlack;				//surrounded player who tries to live. wall will be in the opposite colour. true=Black, false=White
	private boolean placeBlackStones;		//color of single placed stones. true=black,false=white
	private boolean placeStonesAlternating;		//color of single placed stones alternating or not. will change placeBlackStones if true.
	private boolean placeLivingGroupesVertical;		//in this case a living group will be a 5x3-block of stones. for false it's a 3x5-block.
	...
	public LocalAnalysis(String coordinate1, String coordinate2, int playerInside, int playerToMove, int blackKoThreats2) {
		int a = 0;
		...
		copyBoard();
		protectFieldSquareAndSetWall(startCoordinates[0], startCoordinates[1]);	//protects the selected area and sets corner points for the wall
...
//	
	 ...
	 */
			...
			if ( a < BOARDSIZE ) {
				for( int i = xCoord( a ) - 2; i <= xCoord( a ) + 2; i++) {
					for( int j = yCoord( a ); j <= yCoord( a ) + 1; j++) {
						if (j != yCoord( a ) || ( i != xCoord( a ) + 1 && i != xCoord( a ) - 1 ) && i >= 0 && i < BOARDSIZE ) {
			...
			if ( a % BOARDSIZE == 0 ) {
				for( int i = xCoord( a ); i <= xCoord( a ) + 1; i++) {
					for( int j = yCoord( a ) - 2; j <= yCoord( a ) + 2; j++) {
						if (i != xCoord( a ) || ( j != yCoord( a ) + 1 && j != yCoord( a ) - 1 ) && j >= 0 && j < BOARDSIZE ) {
							setProtectedStone( getField( i, j ) );
						}
						else {
							if(j >= 0 && j < BOARDSIZE) {
		...
		/**
		 * moves the border between main areas to change the point difference. if positive Black gains p points. else he looses p points.
		 * @param p number of points Black gains and White looses. If negative Black looses p points and White wins p Points
		 */
					...
					if ( changingPoint % BOARDSIZE != 0 
						...
						if ( changingPoint >= BOARDSIZE 
							...
							if ( changingPoint <= BOARDSIZE * BOARDSIZE - BOARDSIZE 
								...
								else { //we didn't find a changing point in our neighborhood. so we check the whole board
									for( int i = 0; i < BOARDSIZE * BOARDSIZE; i++) {
										if ( area[i] == Area.MAINAREAWHITE ) {
											if ( check4Neighborhood( i, Area.MAINAREABLACK )  
													&& !check4Neighborhood(i, Area.PROTECTEDAREA) 
		...
		/**
		 * Returns to the point in the game there we started the Local Analysis.
		 * Deletes the branch of the Local Analysis
		 */
		public void returnToGame() {
//			int ret =
//		            JOptionPane.showConfirmDialog(
//		                null,
//		                "This will delete all moves and branches of the Local Analysis. Make sure that you are in a Branch of the Local Analysis!",
//		                "Delete",
...
//	        }
		...
		//TODO really load from Configs
				private void loadConfig(int playerInside, int playerToMove, int blackKoThreats2) {
					...
					wallDistance = Lizzie.config.wallDistance;				//between analyzed position and surrounding stones there will be a void so the wall will not affect the local position
					blackKoThreats = blackKoThreats2;		//builds Ko threats for Black if positive and for White if negative
					maxKoThreatsBlack = 0;			//maximal amount of Ko threats for Black. depends on size of Whites eye area.
					...
					placeStonesAlternating = false;		//color of single placed stones alternating or not. will change placeBlackStones if true.
					placeLivingGroupesVertical = true;		//in this case a living group will be a 5x3-block of stones. for false it's a 3x5-block.
		 ...
		 * twists also the board cause we use different coordinates than Lizzie
		 */
		...
		/**
		 * mirror symmetrical
		 * use twice to get the same position
		 */
...
//			startCoordinates[1] = B * yCoord(startCoordinates[1]) + B - 1 - xCoord(startCoordinates[1]);
		...
		/**
		 * spin clockwise
		 * use 4 times to get the same position
		 */
		private void mirror2() {
			...
			startCoordinates[0] = BOARDSIZE * xCoord(startCoordinates[0]) + BOARDSIZE - 1 - yCoord(startCoordinates[0]);
			startCoordinates[1] = BOARDSIZE * xCoord(startCoordinates[1]) + BOARDSIZE - 1 - yCoord(startCoordinates[1]);
			return;
		}
		...
		private boolean createBlackEyes(boolean searchArea) {
			...
			while( (xCoord(startPoint) + width) < BOARDSIZE - 1 && area[startPoint + width + 1] == Area.EYESPACEBLACK ) {
				width++;
			}
			while( yCoord(startPoint) - height > 0 && area[startPoint - height * BOARDSIZE - BOARDSIZE] == Area.EYESPACEBLACK ) {
				height++;
			}
			while( startPoint + height * BOARDSIZE + BOARDSIZE < BOARDSIZE * BOARDSIZE && area[startPoint + height * BOARDSIZE + BOARDSIZE] == Area.EYESPACEBLACK ) {
				height++;
			}
			width++;
			height++;
				...
				case 0:
					...
					if(isCorner(startPoint)) {
						...
						if(startPoint == BOARDSIZE * BOARDSIZE - BOARDSIZE) {
							...
							if ( checkSpaceForLivingGroup ( BOARDSIZE * BOARDSIZE - BOARDSIZE + 1, Area.EYESPACEBLACK ) ) {
								setUnprotectedLivingGroup(BOARDSIZE * BOARDSIZE - BOARDSIZE + 1);
								return true;
							}
							if ( checkSpaceForLivingGroup ( BOARDSIZE * BOARDSIZE - 2 * BOARDSIZE, Area.EYESPACEBLACK ) ) {
		...
		private boolean createWhiteEyes(boolean searchArea) {
			...
			while( (xCoord(startPoint) + width) < BOARDSIZE - 1 && area[startPoint + width + 1] == Area.EYESPACEWHITE ) {
				width++;
			}
			while( yCoord(startPoint) - height > 0 && area[startPoint - height * BOARDSIZE - BOARDSIZE] == Area.EYESPACEWHITE ) {
				height++;
			}
			while( startPoint + height * BOARDSIZE + BOARDSIZE < BOARDSIZE * BOARDSIZE && area[startPoint + height * BOARDSIZE + BOARDSIZE] == Area.EYESPACEWHITE ) {
				height++;
			}
			width++;
			height++;
				...
				case 0:
					...
					if(isCorner(startPoint)) {
						...
						if(startPoint == BOARDSIZE * BOARDSIZE - 1) {
							...
							if ( checkSpaceForLivingGroup ( BOARDSIZE * BOARDSIZE - BOARDSIZE - 1, Area.EYESPACEWHITE ) ) {
								setUnprotectedLivingGroup(BOARDSIZE * BOARDSIZE - BOARDSIZE - 1);
								return true;
							}
						}
		...
		/**
		 * builds eyes for parts of main area which arn't connected to the main part
		 * for example if the main area gets divided in two parts by the Separation group
		 */
							...
							if(playerInsideBlack ? 
								...
								for(int b = 0; b < BOARDSIZE * BOARDSIZE; b++) {
									if(eyelessArea[b] && area[b] == Area.MAINAREABLACK) {
										if(checkSpaceForLivingGroupAndChangeType(b, Area.MAINAREABLACK, Area.EYESPACEBLACK)) {
											placeBlackStones = true;
									...
									if(b == BOARDSIZE * BOARDSIZE - 1) {		//if we can't get the group alive, we will change the color of the group
										printError(36);
										placeBlackStones = false;
										...
										for(int c = 0; c < BOARDSIZE * BOARDSIZE; c++) {
											if(playerInsideBlack) {
												if(connectToArea(c, checkBoard, eyelessArea, Area.PROTECTEDAREA, Stone.WHITE) && 
														connectToArea(c, checkBoard, eyelessArea, Area.EYESPACEWHITE, Stone.WHITE)) {
													//TODO
													printError(37);
												}
											}
											else {
												if(connectToArea(c, checkBoard, eyelessArea, Area.SEPARATIONGROUPS, Stone.WHITE) && 
														connectToArea(c, checkBoard, eyelessArea, Area.EYESPACEWHITE, Stone.WHITE)) {
													//TODO
													printError(37);
												}
							...
							if(!playerInsideBlack ?
								...
								for(int b = 0; b < BOARDSIZE * BOARDSIZE; b++) {
									if(eyelessArea[b] && area[b] == Area.MAINAREAWHITE) {
										if(checkSpaceForLivingGroupAndChangeType(b, Area.MAINAREAWHITE, Area.EYESPACEWHITE)) {
											placeBlackStones = false;
											setUnprotectedLivingGroup(b);
											Arrays.fill(eyelessArea, false);
											break;
										...
										for(int c = 0; c < BOARDSIZE * BOARDSIZE; c++) {
											if(!playerInsideBlack) {
												if(connectToArea(c, checkBoard, eyelessArea, Area.PROTECTEDAREA, Stone.BLACK) && 
														connectToArea(c, checkBoard, eyelessArea, Area.EYESPACEBLACK, Stone.BLACK)) {
													//TODO
													printError(37);
												}
											}
											else {
												if(connectToArea(c, checkBoard, eyelessArea, Area.SEPARATIONGROUPS, Stone.BLACK) && 
														connectToArea(c, checkBoard, eyelessArea, Area.EYESPACEBLACK, Stone.BLACK)) {
													//TODO
													printError(37);
												}
		...
		private boolean createEyeSpaceBlack() {
			...
			if ( allowKo ) {
				...
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 5 * BOARDSIZE, 3, 5, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
					maxKoThreatsWhite = 3;
					return true;
				}
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 3 * BOARDSIZE, 5, 3, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
					maxKoThreatsWhite = 3;
					return true;
				}
				for( int i = 0; i < BOARDSIZE; i++ ) {
					...
					if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 1 - i , 6, 3, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
						maxKoThreatsWhite = 3;
						return true;
					}
					...
					if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 1 - i , 5, 4, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
						maxKoThreatsWhite = 3;
						return true;
					}
				}
				...
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 4 * BOARDSIZE, 3, 4, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
					maxKoThreatsWhite = 2;
					return true;
				}
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 3 * BOARDSIZE, 4, 3, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
					maxKoThreatsWhite = 2;
					return true;
				}
				for( int i = 0; i < BOARDSIZE; i++ ) {
					...
					if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 1 - i , 5, 3, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
						maxKoThreatsWhite = 2;
						return true;
					}
				}
				...
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 5 * BOARDSIZE, 2, 5, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
					maxKoThreatsWhite = 1;
					return true;
				}
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 2 * BOARDSIZE, 5, 2, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
					maxKoThreatsWhite = 1;
					return true;
				}
				for( int i = 0; i < BOARDSIZE; i++ ) {
					...
					if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 1 - i , 6, 2, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
						maxKoThreatsWhite = 1;
						return true;
					}
				}
			...
			if( checkSpaceForLivingGroupAndChangeType( BOARDSIZE * BOARDSIZE - BOARDSIZE, Area.MAINAREABLACK, Area.EYESPACEBLACK ) ) {
				return true;
			}
			for( int a = 0; a < BOARDSIZE * BOARDSIZE; a++ ) {
				if(isBorder(a)) {
		...
		private boolean createEyeSpaceWhite() {
			...
			if ( allowKo ) {
				...
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 5 * BOARDSIZE - 3, 3, 5, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
					maxKoThreatsBlack = 3;
					return true;
				}
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 3 * BOARDSIZE - 5, 5, 3, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
					maxKoThreatsBlack = 3;
					return true;
				}
				for( int i = 0; i < BOARDSIZE; i++ ) {
					if ( checkSquareAndChange( i * BOARDSIZE + BOARDSIZE - 1, 3, 6, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
						maxKoThreatsBlack = 3;
						return true;
					}
					...
					if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 1 - i , 6, 3, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
						maxKoThreatsBlack = 3;
						return true;
					}
					if ( checkSquareAndChange( i * BOARDSIZE + BOARDSIZE - 1, 4, 5, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
						maxKoThreatsBlack = 3;
						return true;
					}
					...
					if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 1 - i , 5, 4, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
						maxKoThreatsBlack = 3;
						return true;
					}
				}
				...
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 4 * BOARDSIZE - 3, 3, 4, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
					maxKoThreatsBlack = 2;
					return true;
				}
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 3 * BOARDSIZE - 4, 4, 3, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
					maxKoThreatsBlack = 2;
					return true;
				}
				for( int i = 0; i < BOARDSIZE; i++ ) {
					if ( checkSquareAndChange( i * BOARDSIZE + BOARDSIZE - 1, 3, 5, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
						maxKoThreatsBlack = 2;
						return true;
					}
					...
					if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 1 - i , 5, 3, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
						maxKoThreatsBlack = 2;
						return true;
					}
				}
				...
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 5 * BOARDSIZE - 2, 2, 5, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
					maxKoThreatsBlack = 1;
					return true;
				}
				if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 2 * BOARDSIZE - 5, 5, 2, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
					maxKoThreatsBlack = 1;
					return true;
				}
				for( int i = 0; i < BOARDSIZE; i++ ) {
					if ( checkSquareAndChange( i * BOARDSIZE + BOARDSIZE - 1, 2, 6, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
						maxKoThreatsBlack = 1;
						return true;
					}
					...
					if ( checkSquareAndChange( BOARDSIZE * BOARDSIZE - 1 - i , 6, 2, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
						maxKoThreatsBlack = 1;
						return true;
					}
				}
			...
			//if we don't allow Ko or don't find enough space for a Ko threat
			maxKoThreatsBlack = 0;
				if( checkSpaceForLivingGroupAndChangeType( BOARDSIZE * BOARDSIZE - 1, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
					return true;
				}
				if( checkSpaceForLivingGroupAndChangeType( BOARDSIZE - 1, Area.MAINAREAWHITE, Area.EYESPACEWHITE ) ) {
					return true;
				}
				for( int a = 0; a < BOARDSIZE * BOARDSIZE; a++ ) {
					if(isBorder(a)) {
				...
				for(int i = 0; i < 8; i++) {
					for(int j = 0; j < BOARDSIZE; j++) {
						if(check8Neighborhood(getField(i, j), Area.PROTECTEDAREA)) {
							if(yCoord(wallCornerPoint[0]) != 0)		//reset Area status of wall to EMPTY so we can use the wall stones for a living group
							...
							if(checkSpaceForLivingGroupAndChangeType2(getField(i, j), Area.PROTECTEDAREA, Area.PROTECTEDAREA)) {
		...
		//TODO more dynamic
		/**
		 * builds eyes for wall
		 * connects as Alive marked Points to the outside and builds eyes for them
		 */
		...
		/**
		 * protects all fields in a square with corner points a and b. also sets points for the surrounding wall later.
		 */
		private void protectFieldSquareAndSetWall(int a, int b) {
			...
			placeBlackStones = !playerInsideBlack;
			if(yCoord(wallCornerPoint[0]) != 0)
				setStoneLine(wallCornerPoint[0], wallCornerPoint[1]);
			if(xCoord(wallCornerPoint[1]) != BOARDSIZE - 1)
				setStoneLine(wallCornerPoint[1], wallCornerPoint[3]);
			if(yCoord(wallCornerPoint[2]) != BOARDSIZE - 1)
				setStoneLine(wallCornerPoint[2], wallCornerPoint[3]);
			if(xCoord(wallCornerPoint[2]) != 0)
				setStoneLine(wallCornerPoint[2], wallCornerPoint[0]);
		}
	...
	private void separateMainAreas() {
		if(playerInsideBlack) {
			placeBlackStones = true;
			int[] separateAreaBlack = new int[50];		//local variable to remember stones we are going to change
			...
			dummy2 = false;
			for( int i = 0; i < k; i++ ) {
				if ( setLivingGroupForSeparation ( separateAreaBlack[i] ) || setLivingGroupForSeparation ( separateAreaBlack[i] - 1 ) ||
					setLivingGroupForSeparation ( separateAreaBlack[i] + BOARDSIZE ) || setLivingGroupForSeparation ( separateAreaBlack[i] - BOARDSIZE ) ) {
					dummy2 = true;
					break;
				}
			}
			if(!dummy2) {		//might lead to different bugs, but worth a try if we didn't find space for our eyes so far 
				dummy2 = true;
				printError(31);
				for( int i = 0; i < k; i++ ) {
					if ( setLivingGroupForSeparation ( separateAreaBlack[i] ) || setLivingGroupForSeparation ( separateAreaBlack[i] - 1 ) ||
						...
						if(!connectToAreaAndChange(a, checkBoard, Area.PROTECTEDAREA, Stone.WHITE)) {
							...
							if(!connectToAreaAndChange(a, checkBoard2, Area.MAINAREAWHITE, Stone.WHITE)) {
								if(!check4Neighborhood(a, Area.PROTECTEDAREA)) {
									if(check4Neighborhood(a + 1, Area.PROTECTEDAREA) && area[a + 1] != Area.SEPARATIONGROUPS) {
										...
										if(check4Neighborhood(a - 1, Area.PROTECTEDAREA) && area[a - 1] != Area.SEPARATIONGROUPS) {
											...
											if(check4Neighborhood(a + BOARDSIZE, Area.PROTECTEDAREA) && area[a + BOARDSIZE] != Area.SEPARATIONGROUPS) {
		...
		//white
			...
			placeBlackStones = false;
			int[] separateAreaWhite = new int[50];;		//local variable to remember stones we are going to change
			int k = 0;		//number of stones we are going to change
			...
			dummy2 = false;
			for( int i = 0; i < k; i++ ) {
				if ( setLivingGroupForSeparation ( separateAreaWhite[i] ) || setLivingGroupForSeparation ( separateAreaWhite[i] + 1 ) ||
					setLivingGroupForSeparation ( separateAreaWhite[i] + BOARDSIZE ) || setLivingGroupForSeparation ( separateAreaWhite[i] - BOARDSIZE ) ) {
					dummy2 = true;
					break;
				}
			}
	...
	private boolean setLivingGroupForSeparation( int a ) {
		...
		if(dummy2 && ((playerInsideBlack && xCoord(a) > BOARDSIZE - 6) || (!playerInsideBlack && xCoord(a) < 6))) {
			return false;
		}
		placeLivingGroupesVertical = true;
	...
	private void restart() {
		...
		private void helpLibertyCheck( int a, boolean[] libertyField, Stone color ) {
			...
			if( color == Stone.EMPTY ) {
				if ( board[a] == Stone.EMPTY ) {
					...
					dummy++;
					if( xCoord( a ) != BOARDSIZE - 1 )
						helpLibertyCheck( a + 1, libertyField, Stone.EMPTY );
					if( xCoord( a ) != 0 )
						helpLibertyCheck( a - 1, libertyField, Stone.EMPTY );
					if( yCoord( a ) != BOARDSIZE - 1 )
						helpLibertyCheck( a + BOARDSIZE, libertyField, Stone.EMPTY );
					if( yCoord( a ) != 0 )
						helpLibertyCheck( a - BOARDSIZE, libertyField, Stone.EMPTY );
					return;
				}
						...
						dummy++;
						if( xCoord( a ) != BOARDSIZE - 1 )
							helpLibertyCheck( a + 1, libertyField, board[a] );
						if( xCoord( a ) != 0 )
							helpLibertyCheck( a - 1, libertyField, board[a] );
						if( yCoord( a ) != BOARDSIZE - 1 )
							helpLibertyCheck( a + BOARDSIZE, libertyField, board[a] );
						if( yCoord( a ) != 0 )
							helpLibertyCheck( a - BOARDSIZE, libertyField, board[a] );
						return;
				}
				...
				if(board[a] == color) {
					...
					dummy++;
					if( xCoord( a ) != BOARDSIZE - 1 )
						helpLibertyCheck( a + 1, libertyField, color );
					if( xCoord( a ) != 0 )
						helpLibertyCheck( a - 1, libertyField, color );
					if( yCoord( a ) != BOARDSIZE - 1 )
						helpLibertyCheck( a + BOARDSIZE, libertyField, color );
					if( yCoord( a ) != 0 )
						helpLibertyCheck( a - BOARDSIZE, libertyField, color );
					return;
				}
		...
		private void changeAreaHelper(int a, boolean[] checkBoard, Area areatype, Area changetype, Stone color) {
			if(!isFieldInBoard(a)) {
				return;
			}
			if(checkBoard[a]) {
		...
		/**
		 * checks if the stone is connected to another stone of type areatype. If so, all the stones on the way will be changed to areatype and it will return true
		...
		private boolean connectToAreaAndChange(int a, boolean[] checkBoard, Area areatype, Stone color) {
			...
			if((xCoord(a) < BOARDSIZE - 1 && connectToAreaAndChange(a + 1, checkBoard, areatype, color)) 
					...
					|| (yCoord(a) < BOARDSIZE - 1 && connectToAreaAndChange(a + BOARDSIZE, checkBoard, areatype, color)) 
		...
		/**
		 * checks if the stone is connected to another stone of type areatype. 
		 * If so, it will return true and mark all connected fields which we checked
		 * @param checkBoard: all the fields we already have gone through
		 * @param returnBoard:	fields which are connected to a Stone of type areatpye
		 */
		private boolean connectToArea(int a, boolean[] checkBoard, boolean[] returnBoard, Area areatype, Stone color) {
			if(!isFieldInBoard(a)) {
				return false;
			}
			if(checkBoard[a]) {
			...
			if((xCoord(a) < BOARDSIZE - 1 && connectToArea(a + 1, checkBoard, returnBoard, areatype, color)) 
					|| (xCoord(a) > 0 && connectToArea(a - 1, checkBoard, returnBoard, areatype, color))
					|| (yCoord(a) < BOARDSIZE - 1 && connectToArea(a + BOARDSIZE, checkBoard, returnBoard, areatype, color)) 
					|| (yCoord(a) > 0 && connectToArea(a - BOARDSIZE, checkBoard, returnBoard, areatype, color))) {
				returnBoard[a] = true;
				return true;
			}
			else {
		...
		/**
		 * checks if the stone is connected to another stone of type areatype. 
		 * If so it will return true. ELSE it will mark all connected fields which we checked
		 * @param checkBoard: all the fields we already have gone through
		 * @param returnBoard:	fields which are connected to a Stone of type areatpye
		 */
		private boolean connectToArea2(int a, boolean[] checkBoard, boolean[] returnBoard, Area areatype, Stone color) {
			if(!isFieldInBoard(a)) {
				return false;
			}
			if(checkBoard[a]) {
		...
		private boolean isCorner (int a) {
			if(a == 0 || a == BOARDSIZE - 1 || a == BOARDSIZE * BOARDSIZE - BOARDSIZE || a == BOARDSIZE * BOARDSIZE - 1) {
				return true;
			}
			return false;
		}
		...
		private boolean isBorder (int a) {
			if(a % BOARDSIZE == 0 || a % BOARDSIZE == BOARDSIZE - 1 || a < BOARDSIZE || a > BOARDSIZE * BOARDSIZE - BOARDSIZE) {
				return true;
			}
			return false;
		}
		...
		/**
		 * checks if all fields which we need for setLivingGroup are from type areatype and change if true every field to changetype
		 */
		...
		/**
		 * checks if all fields which we need for setLivingGroup are NOT from type areatype and change if true every field to changetype
		 */
		...
		private void printField(int a) {
			Lizzie.frame.localAnalysisFrame.gtpConsole.addLine("coordinate:"+String.valueOf(alphabet[a%BOARDSIZE])+(a/BOARDSIZE+1)+"\n");
		}
