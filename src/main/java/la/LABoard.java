...
//  }
  ...
  private void initialize() {
    capturedStones = Lizzie.board.getStones();
    scoreMode = false;
    analysisMode = false;
    playoutsAnalysis = 100;
    saveNode = Optional.empty();
  ...
  public void reopen(int width, int height) {
    ...
    if (width != boardWidth || height != boardHeight) {
      ...
      Zobrist.init();
      clear();
      Lizzie.frame.localAnalysisFrame.leelaz.boardSize(boardWidth, boardHeight);
      Lizzie.frame.localAnalysisFrame.setForceRefresh(true);
    }
  }
    ...
    synchronized (this) {
      ...
      if (history.getNext().map(n -> !n.lastMove.isPresent()).orElse(false) && !newBranch) {
        ...
        if (Lizzie.frame.localAnalysisFrame.isPlayingAgainstLeelaz)
          Lizzie.frame.localAnalysisFrame.leelaz.genmove((history.isBlacksTurn() ? "B" : "W"));
    ...
    synchronized (this) {
      ...
      if (history.next().isPresent()) {
        ...
        if (lastMoveOpt.isPresent()) {
          ...
          Lizzie.frame.localAnalysisFrame.leelaz.playMove(history.getLastMoveColor(), name);
        } else {
          Lizzie.frame.localAnalysisFrame.leelaz.playMove(history.getLastMoveColor(), "pass");
        }
        Lizzie.frame.localAnalysisFrame.refresh();
        Lizzie.frame.localAnalysisFrame.leelaz.endModifyingBoard();
        return true;
  ...
  public void restoreMoveNumber(BoardHistoryNode node) {
    ...
    for (int i = 0; i < stones.length; i++) {
      ...
      if (stone.isBlack() || stone.isWhite()) {
        ...
        Lizzie.frame.localAnalysisFrame.leelaz.playMove(stone, convertCoordinatesToName(x, y));
      }
    }
  ...
  public void clear() {
    ...
    Lizzie.frame.localAnalysisFrame.resetTitle();
    Lizzie.frame.localAnalysisFrame.clear();
    initialize();
  ...
  public boolean previousMove() {
    ...
    synchronized (this) {
      ...
      if (history.previous().isPresent()) {
        Lizzie.frame.localAnalysisFrame.leelaz.undo();
        Lizzie.frame.localAnalysisFrame.refresh();
        Lizzie.frame.localAnalysisFrame.leelaz.endModifyingBoard();
        return true;
      }
  ...
  public void toggleAnalysis() {
    if (analysisMode) {
      Lizzie.frame.localAnalysisFrame.leelaz.removeListener(this);
      analysisMode = false;
    } else {
