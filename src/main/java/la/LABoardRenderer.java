...
public class LABoardRenderer {
  ...
  public LABoardRenderer(boolean isMainBoard) {
  ...
  private void drawGoban(Graphics2D g0) {
    int width = Lizzie.frame.localAnalysisFrame.getWidth();
    int height = Lizzie.frame.localAnalysisFrame.getHeight();
    ...
    if (cachedBackgroundImage.getWidth() != width
        ...
        || Lizzie.frame.localAnalysisFrame.isForceRefresh()) {
      changedName = false;
      cachedBoardWidth = boardWidth;
      cachedBoardHeight = boardHeight;
      Lizzie.frame.localAnalysisFrame.setForceRefresh(false);
  ...
  private void drawName(Graphics2D g0) {
    ...
    drawStringBold(
        ...
        Lizzie.frame.localAnalysisFrame.uiFont,
        black,
        stoneRadius,
  ...
  private void drawStones() {
    ...
    if (cachedStonesImage.getWidth() != boardWidth
        ...
        || cachedBackgroundImageHasCoordinatesEnabled != showCoordinates()
        || !cachedZhash.equals(Lizzie.frame.localAnalysisFrame.board.getData().zobrist)
        || Lizzie.frame.localAnalysisFrame.board.inScoreMode()
        || lastInScoreMode) {
      ...
      cachedZhash = Lizzie.frame.localAnalysisFrame.board.getData().zobrist.clone();
      cachedDisplayedBranchLength = displayedBranchLength;
      cachedBackgroundImageHasCoordinatesEnabled = showCoordinates();
      g.dispose();
      gShadow.dispose();
    ...
    if (Lizzie.frame.localAnalysisFrame.board.inScoreMode()) lastInScoreMode = true;
  }
  ...
  private void drawScore(Graphics2D go) {
    Graphics2D g = cachedStonesImage.createGraphics();
    g.setRenderingHint(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_QUALITY);
    Stone scorestones[] = Lizzie.frame.localAnalysisFrame.board.scoreStones();
  ...
  private void drawBranch() {
    ...
    if (!suggestedMove.isPresent()
        || (!isMainBoard && Lizzie.frame.localAnalysisFrame.isAutoEstimating)
        || (isMainBoard && Lizzie.frame.localAnalysisFrame.isShowingPolicy)) {
      return;
    }
    if (isMainBoard) {
        ...
        .filter(
            move ->
                Board.asCoordinates(move.coordinate)
                    .map(c -> Lizzie.frame.localAnalysisFrame.isMouseOver(c[0], c[1]))
                    .orElse(false))
        .findFirst();
  }
  ...
  private void drawMoveNumbers(Graphics2D g) {
    g.setRenderingHint(KEY_ANTIALIASING, VALUE_ANTIALIAS_ON);
    if (Lizzie.frame.localAnalysisFrame.board == null) return;
    Board board = Lizzie.frame.localAnalysisFrame.board;
    ...
    for (int i = 0; i < Board.boardWidth; i++) {
      for (int j = 0; j < Board.boardHeight; j++) {
        ...
        if (moveNumberList[Board.getIndex(i, j)] > 0
            && (!branchOpt.isPresent() || !Lizzie.frame.localAnalysisFrame.isMouseOver(i, j))) {
  ...
  private void drawLeelazSuggestionsBackgroundShadow(Graphics2D g, List<TextData> textDatas) {
    ...
    if (bestMoves != null && !bestMoves.isEmpty()) {
      ...
      for (int i = bestMoves.size() - 1; i >= 0; i--) {
        ...
        boolean isMouseOver = Lizzie.frame.localAnalysisFrame.isMouseOver(coords[0], coords[1]);
  ...
  private void drawStoneMarkup(Graphics2D g) {
    if (Lizzie.frame.localAnalysisFrame.board == null) return;
    BoardData data = Lizzie.frame.localAnalysisFrame.board.getHistory().getData();
            ...
            m -> {
              if (m.variation.size() > 0) {
                if (Lizzie.frame
                  ...
                  Lizzie.frame.localAnalysisFrame.board.getHistory().previous();
                }
  ...
  public void drawEstimateRect(ArrayList<Double> estimateArray, boolean isZen) {
    ...
    for (int i = 0; i < estimateArray.size(); i++) {
      ...
      Stone stoneHere = Lizzie.frame.localAnalysisFrame.board.getStones()[Board.getIndex(x, y)];
      boolean deadStone =
    ...
    if (Lizzie.frame.localAnalysisFrame.isShowingPolicy
      ...
      for (int n = 0; n < Lizzie.frame.localAnalysisFrame.leelaz.getBestMoves().size(); n++) {
        if (Lizzie.frame.localAnalysisFrame.leelaz.getBestMoves().get(n).policy > maxPolicy)
          maxPolicy = Lizzie.frame.localAnalysisFrame.leelaz.getBestMoves().get(n).policy;
      }
      for (int i = 0; i < Lizzie.frame.localAnalysisFrame.leelaz.getBestMoves().size(); i++) {
        MoveData bestmove = Lizzie.frame.localAnalysisFrame.leelaz.getBestMoves().get(i);
        int y1 = 0;
        int x1 = 0;
