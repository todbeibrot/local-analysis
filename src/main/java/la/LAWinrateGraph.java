...
public class LAWinrateGraph {
  ...
  public void draw(Graphics2D g, int posx, int posy, int width, int height) {
    ...
    int winRateGridLines = Lizzie.frame.localAnalysisFrame.winRateGridLines;
    int midline = 0;
    int midy = 0;
    ...
    while (node.previous().isPresent()) {
      ...
      if (playouts > 0 && node.getData().moveNumber - 1 <= numMoves) {
        ...
        if (Lizzie.frame.localAnalysisFrame.isPlayingAgainstLeelaz
            && Lizzie.frame.localAnalysisFrame.playerIsBlack == !node.getData().blackToPlay) {
          wr = lastWr;
        }
  ...
  private double convertWinrate(double winrate) {
    ...
    if (Lizzie.config.handicapInsteadOfWinrate) {
      double handicap = Lizzie.frame.localAnalysisFrame.leelaz.winrateToHandicap(winrate);
